...
/* Note: TLS 1.3 support requires internal MbedTLS headers that are not
 * available in the public API. TLS 1.3 functionality is disabled. */
#endif
...
static int tls_mbedtls_set_dhparams(struct tls_conf *tls_conf, const struct tls_connection_params *params)
{
    ...
    if (tls_mbedtls_readfile(params->dh_file, &data, &len))
        return 0;
...
static const int suite_HIGH[] = {MBEDTLS_TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256,
                                 ...
                                 /* RSA-PSK ciphersuites not available in MbedTLS 2.x */
...
/* TLS 1.3 support not available in MbedTLS 2.x */
    {
