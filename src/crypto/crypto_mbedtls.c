...
#error "CONFIG_TLS=mbedtls requires MbedTLS 3.6.5 or newer"
#endif
...
/* MD4 is not available in MbedTLS 2.x, use internal implementation */
...
#else /* !MBEDTLS_NIST_KW_C */
/* RFC 3394 AES Key Wrap using mbedtls AES ECB (mbedtls built without
 * MBEDTLS_NIST_KW_C) */
int aes_wrap(const u8 *kek, size_t kek_len, int n, const u8 *plain, u8 *cipher)
{
    ...
    if (mbedtls_aes_setkey_enc(&ctx, kek, kek_len * 8) != 0) {
...
struct crypto_rsa_key *crypto_rsa_key_read(const char *file, bool private_key)
{
    ...
    pk = os_zalloc(sizeof(mbedtls_pk_context));
    if (!pk)
        return NULL;
    ...
    if (private_key) {
        ret = mbedtls_pk_parse_keyfile(pk, file, NULL, hostap_rng_fn, hostap_rng_ctx());
    } else {
        ret = mbedtls_pk_parse_public_keyfile(pk, file);
    }
    ...
    return (struct crypto_rsa_key *)pk;
}
...
{
    /* RC4 is not available in MbedTLS 2.x by default */
    /* This is a placeholder implementation that returns error */
    return -1;
}
#endif /* CONFIG_NO_RC4 */
